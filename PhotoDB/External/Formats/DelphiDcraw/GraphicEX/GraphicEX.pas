...
type
  ...
  TImageProperties = record
    ...
    Width,                             // all images
    Height: LongInt{//DLR Cardinal};                  // all images
  ...
  // DLR
  TPCDSizes = (pcd128,pcd256,pcd512,pcd1024,pcd2048,pcd4096);
  TPCDGraphicData = class(TSharedImage)
  private
  ...
  //DLR
  Private
    FImage:TPCDGraphicData;
    FImageSize:TPCDSizes;
    FNeedImage:Boolean;
  Protected
    Procedure SetImageSize(Val:TPCDSizes);
    ...
    Procedure GetProperties(Const Stream:TStream;Var IWidth,IHeight:Integer);
...
//DLR
    ...
    property ImageSize: TPCDSizes read FImageSize write SetImageSize;
...
Try
  ...
  // DLR added ability to ABORT the loading process
    except
...
// sorted (what we do here).
  ...
  begin
    repeat
      I := L;
      J := R;
      P := FIFD[(L + R) shr 1].Tag; //DLR
      repeat
        while FIFD[I].Tag < P do Inc(I); //DLR
        while FIFD[J].Tag > P do Dec(J); //DLR
        if I <= J then
        begin
          T := FIFD[I];
          FIFD[I] := FIFD[J];
...
Try
  ...
  begin
    ...
    try
      ...
      { dlr
      { If bytecounts=0 and compression=ctnone then
      {    bytecounts=bytesperline
      }
      If (Length(ByteCounts)=0) And (Compression=ctNone) Then
         Begin
         SetLength(ByteCounts,StripCount);
         For X:=0 To StripCount-1 Do
             Begin
             ByteCounts[X]:=BytesPerLine;
             End;
         End;
      while (CurrentStrip < StripCount) And (Length(ByteCounts)<>0) do
      begin
...
procedure TPCDGraphic.DecodeStream(Stream: TStream);
...
Const
  c11:single = 0.0054980*256;
  c12:single = 0.0000000*256;
  c13:single = 0.0051681*256;
  c21:single = 0.0054980*256;
  c22:single =-0.0015446*256;
  c23:single =-0.0026325*256;
  c31:single = 0.0054980*256;
  c32:single = 0.0079533*256;
  c33:single = 0.0000000*256;
Procedure MakeCacheTables;
Var
  X:Integer;
Const
  F=1.0;  
...
For X:=0 To 255 Do
    Begin
    CacheRed1     [X]:=Round((X-156)*C12);
    CacheRed2     [X]:=Round((X-137)*C13*F);
    CacheBlue1    [X]:=Round((X-156)*C32*F);
    CacheBlue2    [X]:=Round((X-137)*C33);
    CacheGreen1   [X]:=Round((X-156)*C22*F);
    CacheGreen2   [X]:=Round((X-137)*C23*F);
    CacheY        [X]:=Round(X*C11*F);
    End;
end;

Procedure Convert(YY,C1,C2,Linebuffer:Pointer;width:integer;mask:integer);
Var
  X:Integer;
  T,PY,PR,PB:PByte;
  BitRun:Byte;
...
While Width>0 Do
      ...
      if Boolean(Mask and BitRun) then
         begin
         R1:=ClampByte(CacheY[PY^]+{CacheRed1[PB^]+}CacheRed2[PR^]);
         G1:=ClampByte(CacheY[PY^]+CacheGreen1[PB^]+CacheGreen2[PR^]);
         B1:=ClampByte(CacheY[PY^]+CacheBlue1[PB^]{+CacheBlue2[PR^]});
         inc(py,1);
         inc(pb,1);
         inc(pr,1);
         T^:=b1;
...
Try
  ...
  begin
    ...
    begin
      ...
      FWidth := 192 shl ImageIndex;
      FHeight := 128 shl ImageIndex;
            ...
            case Rotate of
              1: // rotate -90°
                begin
                  ...
                  begin
// dlr
Convert(YY,C1,C2,Linebuffer,width,$FF);
{
...
Convert(YY,C1,C2,Linebuffer,width,$FF);
{
              ...
              begin
// dlr
Convert(YY,C1,C2,scanline[y],width,$FF);
{
...
// DLR
destructor TPCDGraphicData.Destroy;
begin
  FData.Free;
  inherited Destroy;
end;

procedure TPCDGraphicData.FreeHandle;
begin
end;
...
destructor TPCDGraphic.Destroy;
begin
  FImage.Release;
  inherited Destroy;
end;

procedure TPCDGraphic.Assign(Source: TPersistent);
begin
FImage.Release;
FNeedImage:=False;
  if Source is TPCDGraphic then
     begin
     FImage := TPCDGraphic (Source).FImage;
     FImage.Reference;
     FWidth:=TPCDGraphic(Source).Width;
     FHeight:=TPCDGraphic(Source).Height;
     inherited Assign(Source);
     end
  else
     Begin
     FImage:=Nil;
     FImage:=TPCDGraphicData.Create;
...
procedure TPCDGraphic.AssignTo(Dest: TPersistent);
begin
...
procedure TPCDGraphic.CalcOutputDimensions;
begin
  try
  finally
  end;
...
procedure TPCDGraphic.Changed(Sender: TObject);
begin
  inherited Changed(Sender);
end;

procedure TPCDGraphic.DIBNeeded;
begin
FreeBitMap;
GetBitmap;
Changed(Self);
...
Function TPCDGraphic.GetBitMap:TBitmap;
Begin
If FNeedImage Then
   Begin
   FImage.FData.Position:=0;
   DecodeStream(FImage.FData);
   End;
Result:=TBitMap(Self);
End;

Procedure TPCDGraphic.SetImageSize(Val:TPCDSizes);
Begin
If FImageSize<>Val Then
   ...
   FImage.FData.Position := 0;
   GetProperties(FImage.FData,FWidth,FHeight);
   FImage.FData.Position := 0;
   End;
End;

procedure TPCDGraphic.Draw(ACanvas: TCanvas; const Rect: TRect);
begin
If FNeedImage Then
   Begin
   GetBitMap;
...
procedure TPCDGraphic.FreeBitmap;
begin
end;

function TPCDGraphic.GetEmpty: Boolean;
begin
  Result := (FImage.FData = nil) ;
end;

function TPCDGraphic.GetHeight: Integer;
begin
(*
If FNeedImage Then
   Begin
...
function TPCDGraphic.GetWidth: Integer;
begin
(*
If FNeedImage Then
   Begin
...
Procedure TPCDGraphic.SetHeight(Val: Integer);
Begin
FHeight:=Val;
Inherited;
End;

Procedure TPCDGraphic.SetWidth(Val: Integer);
Begin
FWidth:=Val;
Inherited;
End;

procedure TPCDGraphic.NewBitmap;
Begin
Self.Width:=0;
Self.Height:=0;
FNeedImage:=True;
...
procedure TPCDGraphic.NewImage;
begin
...
Function TPCDGraphic.GetImageIndex:Integer;
Begin
Result:=2;
Case FImageSize Of
       pcd128:Result:= 0; // third image is Base resolution
       pcd256:Result:= 1; // third image is Base resolution
       pcd512:Result:= 2; // third image is Base resolution
       pcd1024:Result:= 3; // third image is Base resolution
       pcd2048:Result:= 4; // third image is Base resolution
       pcd4096:Result:= 5; // third image is Base resolution
       End;
End;

Procedure TPCDGraphic.GetProperties(Const Stream:TStream;Var IWidth,IHeight:Integer);
Var
  ImageIndex:Integer;
  Temp:Integer;
Begin
...
if ReadImageProperties(Stream, ImageIndex) then
   ...
   with Stream, FImageProperties do
    ...
    // since row and columns might be swapped because of rotated images
    // we determine the final dimensions once more
    IWidth := 192 shl ImageIndex;
    IHeight := 128 shl ImageIndex;
    if Rotate in [1, 3] then
       Begin
       Temp:=IHeight;
       IHeight:=IWidth;
...
procedure TPCDGraphic.ReadData(Stream: TStream);
var
  Size: Longint;
begin
...
procedure TPCDGraphic.ReadStream(Size: Longint; Stream: TStream);
begin
  NewImage;
  NewBitmap;
  with FImage do
  ...
  FData.Position := 0;
  GetProperties(FData,FWidth,FHeight);
  FData.Position := 0;
  end;
...
{DLR}
function TPPMGraphic.GetWord: Byte;
Var
  MWord:Integer;
  B1,B2:Byte;
...
begin
  ...
  B2:=Byte(FBuffer[FIndex]);
  Inc(FIndex,1);

  MWord:=Round((B1*256+B2) / 2.5);
  If MWord>255 Then MWord:=255;
  Result:=MWord;
end;
