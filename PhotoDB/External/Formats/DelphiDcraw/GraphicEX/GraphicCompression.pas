...
begin
  ...
  If (Run Div 8)+Integer(FTarget)>Integer(FMaxTarget) Then
     Begin
     RLen:=(Integer(FMaxTarget)-Integer(FTarget));
     If RLen<0 Then RLen:=0;
     Run:=RLen*8;
     End;
...
begin  
  ...
  // target preparation
  FTarget := Dest;
  FMaxTarget:=Pointer(Integer(Dest)+UnPackedSize);
  FRestWidth := FWidth;
  FFreeTargetBits := 8;
  EOLCount := 0;
  ...
  repeat
    ...
    repeat
      ...
      begin
        ...
        If (FTarget - PChar(Dest) >= UnpackedSize) Then Break;
