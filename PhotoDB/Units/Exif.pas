  ...
  try
    ...
    if frac then begin
     if denom<>0 then
      ...
      begin
        if denom/nom>1 then
        Result:=inttostr(1)+'/'+inttostr(Round(denom/nom)) else
        Result:=inttostr(Round(nom/denom));
      end
      else Result:='0';
  except
    Result:='';
...
begin
  ...
  // DateTime
          $0132:
          begin
           FDateTime:=ReadAsci(tag.Offset+off0, tag.Count);

           if TryStrToDate(FDateTime,DT) then
           begin
            FTime:=TimeOf(DT);
            FDate:=DateOf(DT);
           end else
           begin
            D:=Copy(FDateTime,1,10);
            T:=Copy(FDateTime,12,8);
            TryStrToDate(D,FDate);
            if FDate=0 then
            begin
             yyyy:=StrToIntDef(Copy(FDateTime,1,4),0);
             mm:=StrToIntDef(Copy(FDateTime,6,2),0);
             dd:=StrToIntDef(Copy(FDateTime,9,2),0);
             if (yyyy>1990) and (yyyy<2050) then
             if (mm>=1) and (mm<=12) then
             if (dd>=1) and (dd<=31) then
             FDate:=EncodeDate(yyyy,mm,dd);
            end;
            TryStrToTime(T,FTime);
            FTime:=TimeOf(FTime);
           end;
          end;
  // CopyRight
  ...
  //ISO Speed
            $8827: begin if Tag.Offset<=64000 then FISO := Tag.Offset; end;
  // Aperture
