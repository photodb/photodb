...
SET DELPHI=C:\Program Files (x86)\Embarcadero\Studio\15.0
...
cd Uninstall
"%DCC32%" %NS% UnInstall -D"UNINSTALL" -E"..\PhotoDB\bin" -N0"\PhotoDB\dcu" -U"..\Dmitry" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM
cd ..

cd Bridge
"%DCC32%" %NS% PhotoDBBridge -E"..\PhotoDB\bin" -N0"\PhotoDB\dcu" -U"..\Dmitry" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM
cd ..

cd Installer
"%DCC32%" %NS% InstallMaker -D"EXTERNAL;%PLAYER%" -N0"..\dcu" -U"..\Dmitry" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM
cd ..

cd PhotoDB

move photodb.cfg photodb.cfg.safe
"%DCC32%" %NS% photodb -D"PHOTODB;LICENCE" -Ebin -W -N0dcu --inline:on -U"..\Dmitry";"External\Mustangpeak\EasyListview\Source";"External\Mustangpeak\Common Library\Source";"External\Controls\DragDrop\Source";"External\Controls\Image Controls\Source";"External\FastMM";"External\scalemm";"External\Controls\virtual-treeview\Common";"External\jcl\source\windows";"External\jcl\source\common" -I"External\jcl\source\include" -R"Resources";"..\DBIcons";"%DELPHI%\Lib" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM
...
cd KernelDLL
"%DCC32%" %NS% Kernel -E"..\bin" -N0"..\dcu" -U"..\Dmitry" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM
cd ..
...
RC SETUP_ZIP.rc
"%DCC32%" %NS% Install -D"INSTALL;%PLAYER%" -N0"..\PhotoDB\dcu" -U"..\Dmitry" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM

InstallMaker /setup Install.exe
rem use VS resource compiler because borland's fails on big files
RC Install_Package.rc
"%DCC32%" %NS% PhotoDBInstall -E".." -D"INSTALL" -N0"..\PhotoDB\dcu" -U"..\Dmitry" -$I+ -$O+ -W-SYMBOL_PLATFORM -W-UNIT_PLATFORM
