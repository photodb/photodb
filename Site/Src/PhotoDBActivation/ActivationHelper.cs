...
namespace PhotoDBActivation
{
    public class ActivationHelper
    {
        ...
        public static string GenerateFreeActivationNumber(string applicationKey)
        {
            ...
            for (int i = 1 ; i <=  8; i++)
            {
                SSS = ((int)Math.Abs(Math.Round(Math.Cos(Convert.ToByte(sb[i]) + 100 * i + 0.34) * 16))).ToString("X8");
                sb[i] = SSS[8];
            };
            ...
            for (int i = 1 ; i <=  8; i++)
                Ar[(i - 1) * 2 + 1] = Int32.Parse(new string(applicationKey[i], 1), System.Globalization.NumberStyles.HexNumber);
