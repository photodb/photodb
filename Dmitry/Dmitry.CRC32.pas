...
function CalcStringCRC32(s: string; out CRC32: DWORD): Boolean; overload;
function CalcAnsiStringCRC32(s: AnsiString; out CRC32: DWORD): Boolean; overload;
procedure CalcFileCRC32(FromName: string; var CRCvalue: DWORD;
  var TotalBytes: TInteger8;
  var error: Word);
function StringCRC(Str: string): Cardinal; overload;
function StringCRC(Str: AnsiString): Cardinal; overload;
function CalcBufferCRC32(const Buffer; Size: Integer; out CRC32: DWORD; InitialCRC: DWORD = $FFFFFFFF): Boolean;
...
function CalcAnsiStringCRC32(s: AnsiString; out CRC32: DWORD): Boolean;
var
  CRC32Table: DWORD;
begin
{$IFOPT R+}
...
{$ENDIF}
  ...
  if CRC32Table <> $6FCF9E13 then raise Exception.Create('CRC32 Table CRC32 is ' +
...
{$ENDIF}
  ...
  begin
    CRC32 := $FFFFFFFF; // To match PKZIP
    if Length(S) > 0  // Avoid access violation in D4
      then CalcCRC32(Addr(S[1]), Length(s) * SizeOf(s[1]), CRC32);
    CRC32 := not CRC32; // To match PKZIP
  end;
{$IFDEF CKRANGE}
{$UNDEF CKRANGE}
...
begin
  ...
  if CRC32Table <> $6FCF9E13 then
    raise Exception.Create('CRC32 Table CRC32 is ' +
      IntToHex(Crc32Table, 8) +
      ', expecting $6FCF9E13')
  else  
  begin  
    CRC32 := InitialCRC; //$FFFFFFFF; // To match PKZIP
    if Size > 0  // Avoid access violation in D4
      then CalcCRC32(Addr(Buffer), Size, CRC32);
    CRC32 := not CRC32; // To match PKZIP
  end;  
end;
...
function StringCRC(Str: AnsiString): Cardinal;
begin
  Result := 0;
  CalcAnsiStringCRC32(Str, Result);
end;
