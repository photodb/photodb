...
type
  ...
  protected
    ...
    FCacheSize: Integer;
    FPosition: Int64;
    // the current position in the file (relative to the beginning of the file)
    FCacheStart: Int64;
    // the postion in the file of the start of the cache (relative to the beginning of the file)
    FCacheEnd: Int64;
    // the postion in the file of the end of the cache (relative to the beginning of the file)
    FFileName: string;
    FLastError: DWORD;
    procedure HandleError(const Msg: string);
    ...
    function CreateHandle(FlagsAndAttributes: DWORD): THandle; virtual;
      abstract;
  ...
  public
    ...
    constructor Create(const FileName: string; CacheSize: Integer;
      Handle: THandle); overload; virtual;
  ...
  TReadOnlyCachedFileStream = class(TBaseCachedFileStream,
  ...
  public
    constructor Create(const FileName: string; CacheSize: Integer;
      Handle: THandle); overload; override;
    property UseAlignedCache: Boolean read FUseAlignedCache
      write FUseAlignedCache;
    function Read(var Buffer; Count: Longint): Longint; override;
  ...
  public
    constructor Create(const FileName: string;
      CacheSize, ReadStreamCacheSize: Integer;
      ReadStreamUseAlignedCache: Boolean); overload;
    destructor Destroy; override;
    function Read(var Buffer; Count: Longint): Longint; override;
    function Write(const Buffer; Count: Longint): Longint; override;
  end;

  function GetFileSizeEx(hFile: THandle; var FileSize: Int64): BOOL; stdcall; external 'kernel32.dll' name 'GetFileSizeEx' delayed;
...
constructor TBaseCachedFileStream.Create(const FileName: string;
  CacheSize: Integer; Handle: THandle);
...
function TBaseCachedFileStream.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
...
procedure TBaseCachedFileStream.RaiseSystemError(const Msg: string;
  LastError: DWORD);
begin
  raise EStreamError.Create(Trim(Msg + '  ') + IntToStr(LastError));
end;
...
procedure TBaseCachedFileStream.RaiseSystemErrorFmt(const Msg: string;
  const Args: array of const);
begin
...
function TBaseCachedFileStream.FileSeek(const Offset: Int64;
...
begin
  raise ENotImplemented.Create('Read is''t supported');
end;

function TBaseCachedFileStream.Write(const Buffer; Count: Integer): Longint;
begin
  raise ENotImplemented.Create('Write is''t supported');
end;

function TBaseCachedFileStream.Seek(const Offset: Int64;
  Origin: TSeekOrigin): Int64;
...
constructor TReadOnlyCachedFileStream.Create(const FileName: string;
  CacheSize: Integer; Handle: THandle);
begin
  inherited;
...
begin
  ...
  if (ViewStart + ViewLength) > inherited GetFileSize then
    raise ENotImplemented.Create('ViewStart + ViewLength > FileSize');
...
constructor TWriteCachedFileStream.Create(const FileName: string;
  CacheSize, ReadStreamCacheSize: Integer; ReadStreamUseAlignedCache: Boolean);
begin
...
begin
  FlushCache;
  // make sure that the final calls to Write get recorded in the file
...
begin
  ...
  begin
    FReadStream := TReadOnlyCachedFileStream.Create(FFileName,
      FReadStreamCacheSize, FHandle);
...
begin
  ...
  while NumOfBytesLeft > 0 do
  begin
    if ((FPosition < FCacheStart) or (FPosition > FCacheEnd))
    // the current position is outside the cache
    ...
    NumOfBytesToCopy := Min(FCacheSize - (FPosition - FCacheStart),
      NumOfBytesLeft);
    CachePtr := FCache;
    inc(CachePtr, FPosition - FCacheStart);
